...
export default function ClientMarquee() {
  return (
    <section className="py-20 relative overflow-hidden bg-background border-t border-b border-card-border/50">
      ...
      <div className="flex w-[200%] md:w-[150%] animate-[marquee_30s_linear_infinite] hover:[animation-play-state:paused] opacity-60 hover:opacity-100 transition-opacity duration-500">
        <div className="flex w-1/2 justify-around items-center gap-12 px-12">
          {clients.map((client, i) => (
            <div key={i} className="flex flex-col items-center gap-3 grayscale hover:grayscale-0 transition-all duration-300 group">
              <div className="text-text-tertiary group-hover:text-primary transition-colors drop-shadow-[0_0_15px_rgba(var(--primary),0.5)]">
                {client.icon}
              </div>
              <span className="text-xs font-bold tracking-widest uppercase text-text-secondary group-hover:text-foreground transition-colors">
                {client.name}
              </span>
            </div>
          ))}
