...
export default function FranchiseApplyButton({
  recaptchaSiteKey,
  isRecaptchaEnabled
}: {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`${baseUrl}/recruitment/public/franchise`, {
        method: 'POST',
          ...
          >
            ...
            {success ? (
              <div className="text-center py-12">
                ...
                <p className="opacity-70 mb-6">Thank you for your interest. Our partnership team will be in touch shortly.</p>
                <button onClick={() => setOpen(false)} className="btn-primary px-8 py-3">Close</button>
              </div>
            ) : (
              <>
                <h2 className="text-2xl font-bold mb-2">Franchise / Distributor Application</h2>
                <p className="opacity-60 text-sm mb-8">Tell us about your business and expansion plans.</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <input required name="companyName" className="w-full px-4 py-3 rounded-xl bg-background border border-card-border text-sm" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider mb-2 opacity-80">Expected Investment ($)</label>
                      <input required name="investmentBudget" placeholder="e.g. 50,000" className="w-full px-4 py-3 rounded-xl bg-background border border-card-border text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider mb-2 opacity-80">Target Region</label>
                      <input required name="region" className="w-full px-4 py-3 rounded-xl bg-background border border-card-border text-sm" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider mb-2 opacity-80">Message</label>
