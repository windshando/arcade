...
import { createProduct, updateProduct, uploadMediaFile } from '@/app/actions';
...
export default function ProductForm({ categories, initialData }: { categories: any[], initialData?: any }) {
  ...
  const initialMedia = useMemo(() => {
    if (!initialData || !initialData.mediaItems) return [];
    return initialData.mediaItems.map((m: any) => ({
      id: m.mediaFile.id,
      name: m.mediaFile.originalName,
    }));
  }, [initialData]);

  const [mediaList, setMediaList] = useState<{ id: string, name: string }[]>(initialMedia);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Parse initial translations for English
  const englishTranslation = useMemo(() => {
    if (!initialData || !initialData.translations) return null;
    return initialData.translations.find((t: any) => t.locale === 'EN');
  }, [initialData]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-semibold opacity-80 mb-1">Status</label>
              <select name="status" value={formData.status} onChange={handleChange} className="w-full bg-card-bg/50 border border-card-border p-3 rounded-xl focus:ring-1 focus:ring-primary focus:border-primary">
        ...
        <div className="space-y-4">
          <h3 className="font-bold text-lg border-b border-card-border pb-2">Typesetting & Display</h3>
      ...
      <div className="space-y-6 pt-6">
        <div>
          <label className="block text-sm font-semibold opacity-80 mb-2">Short Description</label>
          <div className="bg-card-bg w-full relative z-0">
            <ReactQuill theme="snow" modules={quillModules} value={formData.shortDescription} onChange={(val) => handleQuillChange('shortDescription', val)} className="h-[200px] mb-12" />
          </div>
        </div>

        <div>
          <label className="block text-sm font-semibold opacity-80 mb-2">Full Description</label>
          <div className="bg-card-bg w-full relative z-0">
            <ReactQuill theme="snow" modules={quillModules} value={formData.description} onChange={(val) => handleQuillChange('description', val)} className="h-[400px] mb-12" />
          </div>
        </div>
      </div>
      ...
      <div className="pt-6">
        <h3 className="font-bold text-lg border-b border-card-border pb-2 mb-4">Media Portfolio</h3>
        <div className="border-2 border-dashed border-card-border rounded-xl p-8 text-center bg-card-bg/30 relative z-0">
          <input
            type="file"
            ref={fileInputRef}
